...
// const sample: string =
//   "import React from 'react' const [state, setState] = useState() class Person const person = new Person() var let int float array string number object";
const sample: string =
...
function Typer() {
  ...
  const handleEarlyFinish = useEffect(() => {
    if (input === sample) {
      setIsActive(false);
    }
  }, [input]);
